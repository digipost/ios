...
func track(#error: APIError) {
    ...
    let dictionary = GAIDictionaryBuilder.createEventWithCategory(GoogleAnalyticsConstants.errorCategory, action: errorAction, label: label, value: nil).build() as [NSObject : AnyObject]
    tracker.send(dictionary)
    //    tracker.send(dictionary as! [String : AnyObject])

}

func track(#eventName: String, extraInfo: String?) {
    let tracker = GAI.sharedInstance().defaultTracker
    let dictionary = GAIDictionaryBuilder.createEventWithCategory(GoogleAnalyticsConstants.actionCategory, action: eventName, label: extraInfo, value: nil).build()  as [NSObject : AnyObject]
    tracker.send(dictionary)
